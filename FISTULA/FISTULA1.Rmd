...
These results can play a fundamental role in the understating of fistula maturation and failure causes and are therefore of great interest to hospital and biomedical companies for better patient-selection and treatment. 
...
```{r echo=FALSE}
# Read the hemodynamic data set
ArterialHemodynamics <- read.csv(file="Hemodynamics_Fistula-Artery.csv", head=TRUE, sep=",")
## Load
library(lattice)
par(cex = .6)
options(scipen=999)  # turn-off scientific notation like 1e+48
...
my.strip <- function(which.given, which.panel, ...) {
  ...
  panel.text(x=0.5, y=0.5, adj=c(0.5, 0.55), cex=0.95,
             lab=strip.labels[which.panel[which.given]])}
#Create plots
xyplot(area ~ d.mm | Scan, data = ArterialHemodynamics, groups = Patient, type = "l", lwd=0.5, grid = TRUE,
       strip=my.strip, layout=c(1, 3, 1), xlab="Artery Length [mm]", ylab="Area [mm^2]")
xyplot(WSSave ~ d.mm | Scan, data = ArterialHemodynamics, groups = Patient, type = "l", lwd=0.5, grid = TRUE, 
       strip=my.strip, layout=c(1, 3, 1), xlab="Artery Length [mm]", ylab="Average of WSS [Pa]")
xyplot(WSSmax ~ d.mm | Scan, data = ArterialHemodynamics, groups = Patient, type = "l", lwd=0.5, grid = TRUE, 
       strip=my.strip, layout=c(1, 3, 1), xlab="Artery Length [mm]", ylab="Maximum WSS [Pa]")
xyplot(WSSGave ~ d.mm | Scan, data = ArterialHemodynamics, groups = Patient, type = "l", lwd=0.5, grid = TRUE,
       strip=my.strip, layout=c(1, 3, 1), xlab="Artery Length [mm]", ylab="Average of WSS gradient [1/Pa]")
xyplot(OSI ~ d.mm | Scan, data = ArterialHemodynamics, groups = Patient, type = "l", lwd=0.5, grid = TRUE, 
...
**Full data-sets can be found [here](https://github.com/RosamariaTricarico/2019TDI/tree/master/DATA.SETS), with detailed description at .**  
